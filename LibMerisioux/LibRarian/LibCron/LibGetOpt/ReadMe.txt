...
The "leftover" value shows the post-quoting values of unused words,
separated by spaces. The "leftover_full" value shows the pre-quoting contents
of everything matching that.
...
conventions. The full usage is:
	args, fullargs, argmap, args_to_idx, fullargs_to_idx =
		Library.LibGetOpt.dequote(string)

Where args is the table of arguments (respecting quoting), fullargs is
the split up string ignoring quoting, and argmap is a map from members of
fullargs to the members of args containing them.  The _to_idx tables map
argument indexes to offsets into the original string.

Code and documentation copyright 2007, 2011, 2012, 2013 Peter Seebach. 
Permission granted to use and redistribute under any terms that make you
happy.  I like the BSD license, but it's up to you.
